...
const VotingForm = ({ id }) => {
  ...
  const isRegisterDisabled =
    ...
    (sufragio === 'restringido' && electorList.length === 0)
  ...
  const handleSubmitEdit = (e) => {
    ...
    clearMessages()
    setLoading(true)
    VotingService.updateVotingById(
      id,
      name,
      sufragio,
      startDate,
    ...
    if (id) {
      ...
      VotingService.getVotingById(id).then(
        (response) => {
          ...
          setStartDate(
            new Date(voting.startDate).toISOString().slice(0, 16)
          )
          setEndDate(
            new Date(voting.endDate).toISOString().slice(0, 16)
          )
          setCandidateList(
  ...
  return (
    <section className=' mt-32 h-full w-screen'>
      <div className='px-6 h-full text-gray-800 w-screen'>
        <div className='flex flex-col justify-center align-middle items-center content-center flex-wrap h-full g-6 gap-9'>
          <h1 className=' text-5xl text-center'>
            {id ? 'Editar votación' : 'Crear votación'}
          </h1>
            ...
            >
              <div className='flex sm:flex-row gap-3 mb-1 flex-col'>
                <div className=' sm:w-1/2'>
                  <label htmlFor='name' className='ml-2 text-xs'>
                    Nombre
                  </label>
                  <input
                    type='text'
                    ...
                    value={name}
                    className='mb-1 form-control block w-full px-4 py-2 h-11 text-l font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none'
                    placeholder='Escribe un nombre...'
                  />
                  <label htmlFor='startDate' className='ml-2 text-xs'>
                    Fecha de inicio
                  </label>
                  <input
                    type='datetime-local'
                ...
                <div className=' sm:w-1/2 mb-1'>
                  <label htmlFor='endDate' className='ml-2 text-xs'>
                    Sufragio
                  </label>
                  <select
                    value={sufragio}
